function addToCartList(state,action){
    const {name,_id,subtext,price,originalprice,discount,image,quantity} = action.payload;
    ...
    if(!isProductPresent){
        updatedList=[...state.cartlist,{name,_id,subtext,price,originalprice,discount,image,quantity:quantity-1,qty:1}];
    }
   return updatedList;
}
